...
import '../../data/datasources/mock_academic_api.dart';
import '../../data/models/course.dart';
import '../../data/models/grade.dart';
import '../../data/models/exam.dart';

/// Mock 开关 Provider（主应用可通过 ProviderScope overrides 覆盖）
final academicUseMockProvider = Provider<bool>((ref) => false);

final academicApiProvider = Provider<AcademicApi>((ref) {
  return AcademicApi(ApiClient(config: NetworkConfig.production()));
});

final mockAcademicApiProvider = Provider<MockAcademicApi>((ref) {
  return MockAcademicApi();
});
...
final coursesProvider =
  ...
  if (ref.watch(academicUseMockProvider)) {
    return ref.watch(mockAcademicApiProvider).getCourses(semester: semester);
  }
...
final gradesProvider =
  ...
  if (ref.watch(academicUseMockProvider)) {
    return ref.watch(mockAcademicApiProvider).getGrades(semester: semester);
  }
...
final examsProvider =
  ...
  if (ref.watch(academicUseMockProvider)) {
    return ref.watch(mockAcademicApiProvider).getExams(semester: semester);
  }
...
final semestersProvider = FutureProvider<List<String>>((ref) async {
  if (ref.watch(academicUseMockProvider)) {
    return ref.watch(mockAcademicApiProvider).getSemesters();
  }
...
final coursesForDayProvider =
  ...
  return coursesAsync.maybeWhen(
    data: (courses) => courses
        .where((c) => c.weekday == params.weekday && c.isInWeek(params.week))
        .toList()
...
/// GPA 统计
final gpaStatsProvider =
    Provider.family<Map<String, dynamic>, String?>((ref, semester) {
  ...
  return gradesAsync.maybeWhen(
    data: (grades) {
      ...
      final avgGpa = withGpa.isEmpty
          ? 0.0
          : withGpa.map((g) => g.gpa!).reduce((a, b) => a + b) / withGpa.length;
      return {
